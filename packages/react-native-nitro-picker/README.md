...
Android / iOS 嵌入式原生滚轮选择器。复用 React Native 自带的 Fabric ScrollView 承载自定义 React 选项；Nitro 控制器在 Android UI 线程处理触摸、使用 OverScroller 计算惯性及吸附，在 iOS 使用 UIKit 原生拖动、惯性与目标行吸附。两端同步选中值与中止滚动，无 JS 逐帧驱动，也不新增重复的 Fabric 组件。提供 `PickerView` 和 `PickerViewColumn`，支持多列及由业务管理的数据联动。

首版完整挂载所有选项，没有虚拟化或 React 子树回收，适用于数量适中的选择列表。无弹窗、确认按钮、循环滚动和内置地区数据。

## 接入

通过 workspace 依赖接入，使用 React Native 新架构及包含此包的 Android 或 iOS development build。Nitro / Nitrogen 版本与项目保持一致，为 **0.37.1**。Expo Go 和 Web 不支持挂载；Web 仅导入公开入口不会加载原生实现，尝试挂载时会得到不支持提示。

Nitrogen 绑定位于 `nitrogen/generated`，变更 `.nitro.ts` 接口后执行 `pnpm --filter react-native-nitro-picker codegen:nitro`，生成文件不应手工修改。iOS 通过 `NitroPicker.podspec` 自动链接；原生代码和 Fabric 配置变更需要重新构建开发客户端，Metro 热更新无法替代原生构建。

**本次 iOS 实现未执行原生编译、自动测试或设备验收。** iOS 实际挂载、手势、布局及 VoiceOver 行为仍待设备验证。本仓库执行项目构建需要用户明确授权。
...
`PickerViewColumn` 支持 `children`、`style` 和 `accessibilityLabel`。各列默认等宽，可通过 `style` 中的 `flex` 调整比例。提供可读的 `accessibilityLabel`，以便辅助功能区分各列；列暴露 adjustable 角色、当前索引及递增/递减操作。iOS VoiceOver 的上/下滑调整由 React Native 的无障碍事件调用 Nitro `step`，使用与手势相同的选中和事件路径。

首尾留白使第一项与最后一项均可居中。指示框和遮罩不拦截手势。使用有明确宽度的容器，高度至少为 `itemHeight`，避免把高于行高的内容放入选项。选项仅承载展示内容，内部按钮不接收点击；整个列由滚轮手势和无障碍调整操作控制。

可以嵌入 React Native 的纵向 `ScrollView`。Android 在选择器列内按下后独占该次手势，包括空列、首尾边界及数据变化；在列外开始拖动仍由页面滚动。iOS 使用 UIKit 的原生嵌套滚动手势仲裁，首尾边界与外层页面的竞争需在设备上验收。内部内容容器关闭触摸命中，让滚动视图接收手势；卸载或应用进入后台时原生控制器清理滚动周期。原生手势修改需要重新构建开发客户端后验收。
...
- Android 松手时用 OverScroller 惯性预测目标行，再以连续减速曲线停靠；iOS 在 UIKit 惯性开始前将系统目标偏移对齐至最近行，轻拖后使用系统滚动动画归位。
...
在分别包含此包的 Android 和 iOS 开发客户端检查：快速甩动与首尾吸附、两列同时滚动、受控更新打断滚动、31 日切换二月及闰年、缩减/清空/恢复列、增加删除列、容器尺寸变化、返回页面卸载，以及 TalkBack / VoiceOver 递增递减。分别观察 `immediateChange` 开启和关闭时的事件顺序与去重，并检查图文选项位置和外层页面滚动手势。
